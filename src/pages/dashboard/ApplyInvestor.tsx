...
interface ApplyInvestorProps {
  rejectionReason?: string | null;
}

export default function ApplyInvestor({ rejectionReason }: ApplyInvestorProps) {
  const { user } = useAuth();
            ...
            {step === 1 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input type="text" required value={formData.company_name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, company_name: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">LinkedIn Profile</label>
                  <input type="url" value={formData.linkedin_profile} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, linkedin_profile: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="https://linkedin.com/in/yourprofile" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Why do you want to invest? *</label>
                  <textarea required rows={4} value={formData.reason} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setFormData({...formData, reason: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" />
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <select required value={formData.investment_range} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFormData({...formData, investment_range: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={formData.ticket_size_min} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, ticket_size_min: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="10000" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Max Ticket ($)</label>
                    <input type="number" value={formData.ticket_size_max} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, ticket_size_max: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="100000" />
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Areas of Interest *</label>
                  <textarea required rows={3} value={formData.areas_of_interest} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setFormData({...formData, areas_of_interest: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="Building Materials, Packaging, Textiles" />
                  <p className="text-xs text-gray-500 mt-1">Separate with commas</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Geographic Focus</label>
                  <input type="text" value={formData.geographic_focus} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, geographic_focus: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="North America, Global, East Africa" />
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea rows={4} value={formData.investment_thesis} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setFormData({...formData, investment_thesis: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500" placeholder="Your investment philosophy and focus" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Decision Timeline</label>
                  <select value={formData.decision_timeline} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFormData({...formData, decision_timeline: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
